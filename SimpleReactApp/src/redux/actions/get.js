...
export function/**/ searchMovies(query, sortBy, searchBy, sortOrder, filter){
    ...
    return dispatch => {
        fetch(`https://reactjs-cdp.herokuapp.com/movies?search=${query}&searchBy=${searchBy}&sortBy=${sortBy}&sortOrder=${sortOrder}&filter=${filter}`, {
            method: 'GET',
            headers: {
...
export function* rootSaga() {
    yield takeEvery('UPDATE_MOVIES', searchMovies("", "", "", "asc", ""))
    // yield all([
    //     searchMovies("", "", "", "asc", "")
    // ])
  }